...
def generate_data_optimization(Method,filename,ljcluster,inter,forcemax,maxstep,numberofstructures):
    ...
    for i in range(startnumb,500,1):
        ...
        if Method == 'FIRE':
            ...
            opt = tsase.optimize.SDLBFGS(atoms,logfile=None,maxstep = 0.2, memory=100)
            traj = ase.io.Trajectory(filename, 'a',atoms)
            opt.attach(traj.write,interval=inter)
...
def generate_data_MD(Method,filename,ljcluster,temperature,timestep,forcemax,maxstep,numberofstructures):
    #create new testing/training set
    traj, startnumb = reset_set(filename)
    #loop through the 1000 available starting strucutres
    lj = tsase.calculators.lj(cutoff=15.0)
    ...
    if Method == 'Fire':
        ...
        opt = tsase.optimize.SDLBFGS(atoms,logfile=None,maxstep = 0.2)
        traj = ase.io.Trajectory(filename, 'a',atoms)
        opt.run()        
    ...
    for step in range(numberofstructures):
        ...
        tsase.io.write_con('../ML_AMP_Files/structures/lj5/'+str(step)+'.con',atoms,w='w')
        if len(traj)>numberofstructures:
            break
...
def create_train(Method,trainingset,trainingsetname,trainingstructure,trainingnumber,inter,forcemax,maxstep,temperature,timestep):
	if trainingset == 'optimization':
		generate_data_optimization(Method,trainingsetname,trainingstructure,inter,forcemax,maxstep,trainingnumber)
	elif trainingset == 'MD':
		generate_data_MD(Method,trainingsetname,trainingstructure,temperature,timestep,forcemax,maxstep,trainingnumber)
...
def create_test(Method,testset,testingsetname,testingstructure,testingnumber,inter,forcemax,maxstep,temperature,timestep):
	if testset == 'optimization':
		generate_data_optimization(Method,testingsetname,testingstructure,inter,forcemax,maxstep,testingnumber)
	elif testset == 'MD':
		generate_data_MD(Method,testingsetname,testingstructure,temperature,timestep,forcemax,maxstep,testingnumber)
	elif testset == 'random':

# import all necessary files such as generate, train, ntest, neural network, and loop temperature and time.
import train
import test
...
def generate_data_optimization(filename,ljcluster,inter,forcemax,step,numberofstructures):
    #create new testing/training set
    traj, startnumb = reset_set(filename)#loop through the 1000 available starting strucutres
    cnt = numberofstructures
    for i in range(startnumb,cnt,1):
        lj = tsase.calculators.lj(cutoff=15.0)
        #read in randomly generated starting structures
        atoms = tsase.io.read_con('./structures/lj7_MD/'+str(i)+'.con')
        #ljcluster+'/'+str(i)+'.con'
        atoms.set_calculator(lj)
        atoms.get_potential_energy()
        #write out structures during optimization
        opt = tsase.optimize.SDLBFGS(atoms,maxstep = step, memory = 100)
        traj = ase.io.Trajectory(filename, 'a',atoms)
        opt.attach(traj.write,interval=inter)
        opt.run(fmax=forcemax)
        tsase.io.write_con('./structures/lj7_Optimization/'+str(i)+'.con',atoms,w='w')
        traj.close()
...
def generate_data_MD(filename,ljcluster,temperature,timestep,forcemax,maxstep,numberofstructures):
    ...
    opt = tsase.optimize.SDLBFGS(atoms,maxstep = maxstep, memory = 100)
    traj = ase.io.Trajectory(filename,'a',atoms)
    opt.run() #fmax = forcemax
    ...
    for step in range(numberofstructures):
        dyn.run(100)
        traj.write(atoms)
        tsase.io.write_con('./structures/lj7_MD/'+str(step)+'.con',atoms,w='w')
        if len(traj)>numberofstructures:
            break

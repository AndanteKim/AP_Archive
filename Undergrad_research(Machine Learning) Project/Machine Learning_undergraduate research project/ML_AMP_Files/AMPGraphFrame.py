...
def show_amp(calc,testingsetname,testingnumber,plottitle,plotfigure):
    #creating array for energies calculated with LJ calculator
    amp_array=[]
    #reads in testing set
    traj=ase.io.Trajectory('./structures/lj7_MD/'+testingsetname, 'r')
    numb2=0
    for p in traj:
...
#        lj = tsase.calculators.lj(cutoff=35.0)
        ...
        amp_array.append(p.get_potential_energy())
        #p.set_calculator(lj)
        numb2=numb2+1
    ...
    Max_len = np.amax(amp_array)
    Min_len = np.amin(amp_array)
...
#    plt.clf()
    ...
    fig, ax = plt.subplots()
    # make a Lennardjones histogram : bins_conf, bins, patches
    n, bins, patches = plt.hist(amp_array,bins=bins_conf, range=(Min_len,Max_len), color = 'skyblue', alpha = 0.5, rwidth = 0.85, histtype='bar')
    ...
    plt.xlabel('Energy Calculated using AMP Potential (eV)')
